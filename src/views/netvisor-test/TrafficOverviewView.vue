  ...
  <div class="nv-border-card">
    <div class="nv-tabs-header">
      <div class="nv-tabs-nav" role="tablist">
        <button type="button" class="nv-tab active" role="tab" aria-selected="true">流量概况</button>
      </div>
    </div>
...
async function refresh() {
  try {
    const [bwResp, statsResp, flowsResp] = await Promise.all([
    ...
    // Protocol proportions from recent-flows sample, scaled up to the true
    // active-flow gauge so sub-counts sum to the headline "连接数".
    let tcpConns = 0, udpConns = 0, icmpConns = 0
    const srcIps = new Set<string>()
    flows.forEach((f: any) => {
      const proto = (f.five_tuple?.protocol || f.protocol || '').toUpperCase()
      if (proto === 'TCP' || proto === '6') tcpConns++
      else if (proto === 'UDP' || proto === '17') udpConns++
      else if (proto.includes('ICMP')) icmpConns++
      if (f.five_tuple?.src_ip) srcIps.add(f.five_tuple.src_ip)
    })
    const totalConns = (statsResp as any)?.capture_flow_count || flows.length
    const sample = tcpConns + udpConns + icmpConns
    const scale = sample > 0 ? totalConns / sample : 0
    const tcpScaled = Math.round(tcpConns * scale)
    const udpScaled = Math.round(udpConns * scale)
    const icmpScaled = Math.round(icmpConns * scale)

    // Update stat cards
    setText('toUpstream', fmtBps(curBps))
    setText('toDownstream', fmtBps(peakBps))
    ...
    setText('toUsersCurrent', fmtNum(srcIps.size))
    setText('toUsersPeak', fmtNum(Math.max(srcIps.size, 1)))
    ...
    if (samples.length) {
      ...
      if (upEl) upsertAreaChart('bandwidth', upEl as HTMLElement, xLabels, totalMbpsData, '#91cc75', 'Mbps')

      // 包速率趋势 (pps)
      const downEl = document.querySelector('[data-chart="downstream"] .to-chart-plot-tall')
      if (downEl) upsertAreaChart('pps', downEl as HTMLElement, xLabels, ppsData, '#5470c6')

      // 流量概况 combo (Mbps + PPS)
      const comboEl = document.querySelector('[data-chart="overview"] .to-panel-chart-combo')
      if (comboEl) upsertComboChart('overview', comboEl as HTMLElement, xLabels, totalMbpsData, ppsData)

      // 连接趋势 (PPS proxy)
      const connEl = document.querySelector('[data-chart="connections"] .to-chart-plot-tall')
      if (connEl) upsertAreaChart('connections', connEl as HTMLElement, xLabels, ppsData, '#ee6666')
    }
    ...
    if (trafficPieEl) {
      upsertPieChart('traffic-pie', trafficPieEl as HTMLElement, [
        { name: 'TCP', value: tcpConns || 1 },
        { name: 'UDP', value: udpConns || 1 },
        { name: 'ICMP', value: icmpConns || 1 },
      ].filter(d => d.value > 0))
    }
