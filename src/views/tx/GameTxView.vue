<template>
  <div class="page">
    <el-row :gutter="14" class="kpi-row">
      <el-col :xs="12" :md="6"><StatCard label="游戏会话" :value="formatNumber(rows.length)" sub="手游 / PC / 云游戏" :icon="Trophy" tone="#eab308" /></el-col>
      ...
      <el-col :xs="12" :md="6"><StatCard label="总流量" :value="formatBytes(totalBytes)" sub="游戏字节" :icon="DataLine" tone="#64748b" /></el-col>
    </el-row>
    ...
    <el-table :data="filtered" size="small" stripe :max-height="560" empty-text="暂无游戏事务…"
...
import { Connection, DataLine, Monitor, Refresh, Search, Trophy } from '@element-plus/icons-vue'
import StatCard from '@/components/StatCard.vue'
import { api } from '@/api/client'
import type { Flow } from '@/api/types'
import { formatBytes, formatDurationMs, formatNumber, formatTime, prettyIP } from '@/utils/format'

const GAME_APPS = new Set([
  'Steam', 'Xbox', 'PlayStation', 'Nintendo', 'Epic.Games',
  'Riot.Games', 'LeagueOfLegends', 'Valorant',
  'Genshin.Impact', 'Mihoyo',
  'PUBG', 'Fortnite', 'Apex.Legends', 'Overwatch',
  'MineCraft', 'Roblox',
  'WorldOfWarcraft', 'Blizzard',
  'EA.Games', 'Ubisoft',
  'Tencent.Games', 'NetEase.Games',
  'MobileGame', 'CloudGaming',
])

const allFlows = ref<Flow[]>([])
const search = ref('')

function isGame(f: Flow): boolean {
  const app = f.app_proto || f.master_proto || ''
  return GAME_APPS.has(app)
}

async function load() {
  try {
    const items = await api.recentFlows(500)
    allFlows.value = items.filter(isGame)
  } catch { /* */ }
}
