<template>
  <div class="page">
    <el-row :gutter="14" class="kpi-row">
      <el-col :xs="12" :md="6"><StatCard label="金融会话" :value="formatNumber(rows.length)" sub="支付 / 网银 / 证券" :icon="Wallet" tone="#16a34a" /></el-col>
      ...
      <el-col :xs="12" :md="6"><StatCard label="总流量" :value="formatBytes(totalBytes)" sub="金融字节" :icon="DataLine" tone="#64748b" /></el-col>
    </el-row>
    ...
    <el-table :data="filtered" size="small" stripe :max-height="560" empty-text="暂无金融事务…"
...
import { Connection, DataLine, Monitor, Refresh, Search, Wallet } from '@element-plus/icons-vue'
import StatCard from '@/components/StatCard.vue'
import { api } from '@/api/client'
import type { Flow } from '@/api/types'
import { formatBytes, formatDurationMs, formatNumber, formatTime, prettyIP } from '@/utils/format'

const FINANCE_APPS = new Set([
  'Alipay', 'WeChatPay', 'UnionPay',
  'ICBC', 'CCB', 'ABC', 'BOC', 'CMB',
  'PayPal', 'Stripe', 'Apple.Pay', 'Google.Pay',
  'TongHuaShun', 'EastMoney', 'Flush',
  'ChinaSecurities', 'Futu', 'Tiger.Brokers',
  'Ant.Fortune', 'Tian.Tian.Fund',
])

const allFlows = ref<Flow[]>([])
const search = ref('')

function isFinance(f: Flow): boolean {
  const app = f.app_proto || f.master_proto || ''
  return FINANCE_APPS.has(app)
}

async function load() {
  try {
    const items = await api.recentFlows(500)
    allFlows.value = items.filter(isFinance)
  } catch { /* */ }
}
