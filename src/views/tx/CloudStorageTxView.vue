<template>
  <div class="page">
    <el-row :gutter="14" class="kpi-row">
      <el-col :xs="12" :md="6"><StatCard label="云存储会话" :value="formatNumber(rows.length)" sub="网盘 / 云同步" :icon="UploadFilled" tone="#0ea5e9" /></el-col>
      ...
      <el-col :xs="12" :md="6"><StatCard label="总流量" :value="formatBytes(totalBytes)" sub="云存储字节" :icon="DataLine" tone="#64748b" /></el-col>
    </el-row>
    ...
    <el-table :data="filtered" size="small" stripe :max-height="560" empty-text="暂无云存储事务…"
...
import { Connection, DataLine, Monitor, Refresh, Search, UploadFilled } from '@element-plus/icons-vue'
import StatCard from '@/components/StatCard.vue'
import { api } from '@/api/client'
import type { Flow } from '@/api/types'
import { formatBytes, formatDurationMs, formatNumber, formatTime, prettyIP } from '@/utils/format'

const CLOUD_APPS = new Set([
  'BaiduNetdisk', 'AliYunDrive', '115Netdisk',
  'OneDrive', 'Google.Drive', 'iCloud',
  'Dropbox', 'Box', 'Mega', 'pCloud',
  'WeTransfer', 'SendAnywhere',
  'Tencent.Weiyun', 'Huawei.Cloud',
  'NAS', 'Synology', 'QNAP', 'WebDAV',
])

const allFlows = ref<Flow[]>([])
const search = ref('')

function isCloud(f: Flow): boolean {
  const app = f.app_proto || f.master_proto || ''
  return CLOUD_APPS.has(app)
}

async function load() {
  try {
    const items = await api.recentFlows(500)
    allFlows.value = items.filter(isCloud)
  } catch { /* */ }
}
